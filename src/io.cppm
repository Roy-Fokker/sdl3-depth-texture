...
export namespace io
{
	...
	{
		std::println("{}Reading file: {}{}", colors::BLU, filename.string(), colors::RESET);
	...
	auto read_image_file(const std::filesystem::path &filename) -> image_data
	{
		...
		auto to_sdl_format = [](ddsktx_format format) -> SDL_GPUTextureFormat {
			...
			assert(false && "Unmapped ddsktx_format, no conversion available");
		...
		auto result = ddsktx_parse(&image_data_header,
		                           ...
		                           image_data_size,
		                           &image_parse_error);
		assert(result == true && "Failed to parse image file data.");
